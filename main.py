def choose_action(phonebook):
    while True:
        print('Какое действие вы хотите выбрать?')
        user_choice = input('1 - Найти контакт\n2 - Добавить контакт\n\
3 - Изменить контакт\n4 - Удалить контакт\n5 - Просмотреть все контакты\n0 - Выйти из приложения\n')
        print()
        if user_choice == '1':
            contact_list = read_file_dict(phonebook)
...
def get_new_number():
    ...
    comment_people = input('Введие коментарий: ')
    return last_name, first_name, phone_number, comment_people
...
def read_file_dict(file_name):
    ...
    headers = ['Фамилия', 'Имя', 'Номер телефона', 'Коментарий']
    contact_list = []
    for line in lines:
...
def delete_contact(file_name):
    contact_list = read_file_list(file_name)
